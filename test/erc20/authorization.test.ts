...
describe("[ERC20] Authorization", async () => {
  ...
  it("On-chain sell authorization", async () => {
    ...
    await expect(
      solutionProxy.connect(carol).solveERC20(
        intent,
        {
    ...
    await expect(
      solutionProxy.connect(carol).solveWithOnChainAuthorizationCheckERC20(
        intent,
        {
          data: defaultAbiCoder.encode(["uint128"], [0]),
          fillAmount: intent.amount,
  ...
  it("Signature sell authorization", async () => {
    ...
    // Authorization must come from the intent solver
    {
      ...
      await expect(
        solutionProxy.connect(carol).solveWithSignatureAuthorizationCheckERC20(
          intent,
          {
            data: defaultAbiCoder.encode(["uint128"], [0]),
            fillAmount: intent.amount,
